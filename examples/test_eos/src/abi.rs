...
fn add(
    ...
    table.find(account).is_none().assert(
        "Address for account already exists",
    );
...
fn remove(account: AccountName) {
    ...
    let cursor = addresses.find(account).assert(
        "Address for account not found",
    );
...
fn like(account: AccountName) {
    ...
    let mut address = cursor.get().assert("read");
    address.liked += 1;
    cursor.update(Some(address.account), &address).assert(
        "write",
    );
}
