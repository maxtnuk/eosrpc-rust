...
impl EcTools {
    pub fn new() -> Self {
        ...
        EcTools { scurve_params: curvparams }
    }
    ...
    pub fn integer_to_vec(s: BigInt, length: usize) -> Vec<u8> {
        let b_data = s.to_bytes_be().1.clone();
        if length < b_data.len() {
            b_data
                .iter()
                .skip(b_data.len() - length)
                .map(|x| *x)
                .collect()
    ...
    pub fn vec_to_integer<'a>(d: Data<'a>) -> BigInt {
        BigInt::from_bytes_be(Sign::Plus, d.as_ref())
    }
}
