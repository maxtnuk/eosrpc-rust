...
use errorhandle::{Errortype,err_print};
                ...
                if self.x.clone().modpow(&legendre_exponent,&self.q) == BigInt::one(){
                  ...
                  while U == BigInt::one() || U == q_minus.clone(){
                      ...
                      while p >= self.q.clone() || 
                            (p.clone()*p.clone() - four_q.clone()).modpow(&legendre_exponent,&self.q) != q_minus || 
                            p == BigInt::zero(){
                  ...
                  if let Some(res) = result_str {
                      ...
                      Err(Errortype::MakeFail{who: "EcFieldElement".to_string(),
                                            content: None})
                  }
                }
            }
...
impl EcCurve{
    ...
    pub fn new_from_str( pInHex: &str,
                     aInHex: &str,
                     bInHex: &str) -> Self{
        let (p,a,b) =  (BigInt::parse_bytes(pInHex.as_bytes(), 16).unwrap(),
    ...
    pub fn new_point_fromx(&self,is_odd:bool,x: BigInt)->EcPoint{
        ...
        let alpha = (pow(x.clone(),3) + (self.a.x.clone() * x.clone()) + self.b.x.clone()).mod_floor(&self.q);
    ...
    pub fn decode_point<S>(&self, encoded: S)->Result<EcPoint,Errortype>
    where S: Into<Data>{
        let encoded = encoded.into();
        match encoded[0]{
            0x00 =>{
                Ok(self.new_infinity_point())
            },
            0x02 | 0x03 =>{
                ...
                match alpha.sqrt(){
                    Ok(va) =>{
                        ...
                        if check == ytilde{
                           ...
                           let tmp = EcFieldElement::new(self.q.clone() - va.x.clone(),self.q.clone());
            ...
            0x04 | 0x06 | 0x07 =>{
                ...
                //println!("after \ngx:{}\ngy:{}",,hex::encode(yEnc));
                let (big_xenc,big_yenc) = (BigInt::parse_bytes(en_x.as_bytes(),16).unwrap(),
                                           BigInt::parse_bytes(en_y.as_bytes(),16).unwrap());
                Ok(EcPoint::new(self.clone(),
...
impl EcPoint{
    ...
    pub fn get_encoded(&self)->Option<Data>{
       self.get_encoded_by_bool(self.compressed)
    }
    pub fn get_encoded_by_bool(&self,check: bool)->Option<Data>{
         if self.is_infinity() {
            None
        }else{
            ...
            if check{
                ...
                let pc =
                if bitvecY.get(0).unwrap(){
                    0x03
                }else{
                    0x02
                };
                
                let mut x: Data = EcTools::integer_to_vec(x_clone.clone().x,length);
                x.insert(0,pc);
                Some(x)
            }else{
               let x :Data = EcTools::integer_to_vec(x_clone.clone().x,length);
               let y :Data= EcTools::integer_to_vec(y_clone.clone().x,length);
               let mut result: Data=x.iter().chain(y.iter()).map(|x| *x).collect();
    ...
    pub fn add(self,b: Self) -> Self{
        if self.is_infinity(){
            ...
            if x_clone.clone() == b.x.clone().unwrap(){
                ...
                let gamma = (b.y.unwrap().clone() - y_clone.clone()) / (b.x.clone().unwrap() - x_clone.clone());
    ...
    pub fn twice(self)-> Self{
        if self.is_infinity(){
            ...
            if y_clone.clone().x == BigInt::zero(){
                ...
                let gamma = (x_clone.clone().square() * THREE.clone() + self.curve.a.clone()) / (y_clone.clone() * TWO.clone());
