...
fn key_unit_test(){
  ...
  let (test_str,pub_str)=("5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss",
...
fn random_key_test(){
    ...
    println!("\nprivate key: {}\npublic key: {}",randomkey.to_string(),pub_ran.to_string());
    println!("private valid {}",PrivateKey::is_valid(randomkey.to_string()));
    println!("public valid {}",PublicKey::is_valid(pub_ran.to_string(),None,None));
...
fn valid_test(){
    ...
    let keys: Vec<(bool, &str, Option<&str>)> = vec![
      (true, "PUB_K1_859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2Ht7beeX", None),
      (true, "EOS859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM", None),
      (false, "MMM859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM", None),
      (false, "EOS859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVm", Some("EOS") ),
      (true, "PUB859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM", Some("PUB") ),
      (false, "PUB859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVm", Some("PUB") ),
    ];
    for (b, key, prefix) in keys{
       assert!(b==PublicKey::is_valid(key.to_string(),None,prefix),key.to_string());
    }
...
fn signature_test(){
    ...
    let sigs =vec![Signature::sign(data,&pvt),
