...
impl PublicKey{
    pub fn new<'a>(q: &Any,ctype: Option<CurveType>,prefix: Option<&'a str>) -> Result<Self,Errortype>{
        ...
        if let Some(string) = q.downcast_ref::<String>() {
            ...
            match Self::from_string(curve,string.clone(),in_str){
                ...
                Err(err)=>{
                    err_print(err);
                    Err(Errortype::MakeFail{who: "PublicKey".to_string(),
                                            content: Some(string.clone())})
                }
            }
        } else if let Some(vecs) = q.downcast_ref::<Data>(){
    ...
    fn from_string(curve: EcCurve,public_key: String,prefix: &str)->ResultPoint{
        ...
        let decode_result=
        if !re.is_match(public_key.as_str()){
            let prefix_match=Regex::new(("^".to_owned()+prefix).as_str()).unwrap();
            let result=
            if prefix_match.is_match(public_key.as_str()){
                public_key.clone().split_off(prefix.len())
            }else{
                public_key.clone()   
            };
            keyutil::check_decode(result.as_bytes(),None)
        }else{
            ...
            keyutil::check_decode(keystring.as_bytes(),
                                 Some(keytype.to_string()))
        };
        match decode_result{
            Ok(val) =>{
                 Self::from_data(curve,val.as_slice())       
            },
            Err(err)=>{
                err_print(err);
                Err(Errortype::MakeFail{who: "PublicKey".to_string(),
    ...
    fn from_data<S>(curve: EcCurve,data: S)->ResultPoint
    where S: Into<Data>{
    ...
    pub fn to_uncompressed(&self)->Result<Self,Errortype>{
        ...
        match Self::from_data(curve,data.as_slice()){
            Ok(inner)=>{
...
impl PrivateKey{
    ...
    fn parse(private_key:String) ->ResultParse{
        ...
        if !re.is_match(private_key.as_str()){
            match keyutil::check_decode(private_key.as_bytes(),
                ...
                Ok(versionkey)=>{
                    ...
                    let privateKey = Self::from(versionkey[1..].to_vec());
            ...
            match re.captures(private_key.as_str()){
                Some(key_info)=>{
                    ...
                    match keyutil::check_decode(keystring.as_bytes(),
                                                Some(keytype.to_string())){
                        Ok(pri_data)=>{
                            let privateKey = Self::from(pri_data);
                            Ok((privateKey,"PVT".to_string(),key_info["keytype"].to_string())) 
                        },   
                        Err(err)=>{
                           err_print(err);
                            Err(Errortype::ParseFail)
    ...
    fn to_wif(&self)->String{
        ...
        keyutil::check_encode(private_key,Some("sha256x2".to_string()))
    }
    ...
    pub fn get_shared_secret(&self, pubkey: &PublicKey)->Data{
        let KB=pubkey.to_uncompressed().unwrap();
        let before_slice=KB.get_mdata().multiply(self.mprivate_key.clone()).x.clone().unwrap();
        hash::sha512(before_slice.get_x().to_bytes_be().1.as_slice()).to_vec()
    }
    fn get_child_key(&self,name: String)->Self{
        let mut index=self.to_vec();
        index.extend(name.as_bytes());
        Self::new(hash::sha256(index.as_slice()),self.get_curvetype())
    }
    pub fn from_seed(seed: &str,ctype: Option<CurveType>)->Self{
         Self::new(hash::sha256(seed.as_bytes()),ctype)
    }
...
impl<'a> From<&'a PrivateKey> for PublicKey{
    fn from(prikey: &'a PrivateKey) -> Self {
        let q= prikey.curveparam.get_g().multiply(prikey.mprivate_key.clone());
        let curve=prikey.curveparam.get_curve();
        let data: EcPoint=EcPoint::new(curve,q.x,q.y,true);
        PublicKey::new(&data.get_encoded().unwrap(),
                       prikey.get_curvetype(),None).unwrap_or(Default::default())
    }
}
