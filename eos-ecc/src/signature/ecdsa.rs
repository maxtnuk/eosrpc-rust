...
pub fn deterministic_generate_k<F>(curve: &CurveParam,
                        hashed: Data,
                        d: BigInt,
                        mut checksig: F,
                        nonce: i64) -> Result<BigInt,Errortype>
where F: FnMut(BigInt) ->bool{
    let in_hash=
    if nonce!=0{
        let zeros=vec![0u8;nonce as usize];
        let input: Data = hashed.par_iter().chain(zeros.par_iter()).map(|x| *x).collect();
        hash::sha256(input.as_slice()).to_vec()
    }else{
        hashed.to_vec()
    };
    if in_hash.len() != 32{
...
where T: Into<Data>{
    ...
    let _k = deterministic_generate_k(curve,hashed,d.clone(),|b|{
        ...
        if q.is_infinity() {
            ...
            if r == BigInt::zero(){
                ...
                s = (modinverse(b.clone(),n.clone()).unwrap() * (e.clone() + d.clone() * r.clone())).mod_floor(&n);
                s != BigInt::zero()
            }
        }
    },nonce);
...
pub fn recover_pubkey(curve: CurveParam,e: BigInt,sig: EcSignature,i: BigInt)->Result<EcPoint,Errortype>{
...
pub fn calc_pubkey_recovery_param(curve: &CurveParam,
                                  e: BigInt,
                                  sig: &EcSignature,
                                  pubkey: &PublicKey)->Result<u32,Errortype>{
