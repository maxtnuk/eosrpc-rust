...
impl EcSignature{
    ...
    pub fn from_raw(r: BigInt,
                s: BigInt,
                curve: CurveParam)->Self{
         EcSignature{
            r:r,
            s:s,
            curve:curve
    ...
    pub fn to_compact(&self,i:u8,compressed:bool)->Data{
        ...
        ebuf.par_iter().chain(vec_r.par_iter()).chain(vec_s.par_iter()).map(|x| *x).collect()
    }
    pub fn to_der(&self)-> Data{
        ...
        let mut before_data:Data = vec_r.par_iter().chain(vec_s.par_iter()).map(|x| *x).collect();
    ...
    pub fn to_script_signature(&self,hashtype: u8)->Data{
        ...
        self.to_der().par_iter().chain(hashtypebuffer.par_iter()).map(|x| *x).collect()
    }
    //{compressed,i,signature}
    pub fn parse_compact(buffer: Data,ctype: Option<CurveType>)->Result<(bool,u8,Self),Errortype>{
        if buffer.len()!=65{
            ...
            if i == i & 7{
                ...
                let (r,s)=(BigInt::from_bytes_be(Sign::Plus, rb),
    ...
    pub fn from_der(buffer: Data,ctype: Option<CurveType>)-> Self{    
        ...
        assert!(buffer[1]==buffer.len() as u8-2,"Invalid sequence length");
        assert!(buffer[2]==0x02,"Expected a DER integer");
        
        let rlen = buffer[3];
        assert!(rlen>0,"R length is zero");
        let mut offset=4+rlen;
        assert!(buffer[offset as usize]==0x02,"Expected a DER integer (2)");
    ...
    pub fn parse_script_signature(buffer: Data,ctype: Option<CurveType>)->Result<(Self,u8),Errortype>{
