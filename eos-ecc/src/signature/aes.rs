...
pub fn encrypt(private_key: &PrivateKey,
           public_key: &PublicKey,
           message: Data,
           nonce:Option<i64>)->Data{
    let in_nonce=
    match nonce{
        Some(val)=>{
            val
    ...
    crypt(private_key,public_key,message,in_nonce,None).0.unwrap()
}
pub fn decrypt(private_key: &PrivateKey,
           public_key: &PublicKey,
           message: Data,
           nonce: i64,
           checksum: u32)->Data{
    crypt(private_key,public_key,message,nonce,Some(checksum)).0.unwrap()
}
//(message,checksum)
fn crypt(private_key: &PrivateKey,
         public_key: &PublicKey,
         message: Data,
         nonce: i64,
         checksum:Option<u32>)->(Result<Data,Errortype>,u32){
...
fn crypto_encrypt(message: Data,key: &[u8],iv: &[u8])->Data{
   ...
   
    {
        ...
        cipher.encrypt(&mut input_buffer,&mut output_buffer,false).unwrap();   
    }
...
fn crypto_decrypt(message: Data,key: &[u8],iv: &[u8])->Data{
    ...
    
    {
        let mut output_buffer=RefWriteBuffer::new(&mut output);
        let mut input_buffer=RefReadBuffer::new(message.as_slice());
        decipher.decrypt(&mut input_buffer,&mut output_buffer,false).unwrap();   
    }
