...
impl Signature{
    ...
    fn recoverhash(&self,hashed: Data)->Result<PublicKey,Errortype>{
        ...
        match ecdsa::recover_pubkey(self.sig.curve.clone(),e,self.sig.clone(),i2){
            Ok(val) =>{
                Ok(PublicKey::new_raw(self.sig.curve.clone(),val))
            },
            Err(err)=>{
                err_print(err);
    ...
    where T: Into<Data>{
        ...
        let (i,ecdsig) =loop {
            ...
            let in_ecdsig = ecdsa::sign(&curve,
                                         hashed.clone(),
                                         pvkey.get_mdata(),
                                         nonce);
            nonce+=1;
            ...
            if len_r == 32 && len_s ==32{
                match ecdsa::calc_pubkey_recovery_param(&curve,
                                                     e.clone(),
                                                     &in_ecdsig,
                                                     &pubkey){
                    Ok(val) =>{
...
impl ToString for Signature{
    fn to_string(&self)->String{
        "SIG_K1_".to_string() + keyutil::check_encode(self.to_vec(),Some("K1".to_string())).as_str()
    }
}