...
impl SignProvider{
    ...
    where T: Into<Vec<u8>>{
        ...
        let pubkeys: Vec<String>=self.keys.iter()
                        .filter(|x| x.public_key.clone().is_some())
                        .map(|ref x| x.public_key.clone().unwrap().to_string()).collect();
        ...
        let respond=network.http_request("get_required_keys",&req_value).unwrap();
        println!("{}",respond);
