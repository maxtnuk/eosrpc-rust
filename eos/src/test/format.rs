...
fn format() {
    ...
    assert!(
        "572d3ccdcd".to_string() == encode_name_hex("transfer".to_string()),
        "encode"
    );
    assert!(
        decode_name_hex("572d3ccdcd".to_string(), true) == "transfer".to_string(),
        "decode"
    );
    struct TestSet<'a> {
        value: &'a str,
    ...
    let parsetest = vec![
        ...
        ParseSet {
            text: "1.0000 SYM@tract.token",
            amount: Some("1.0000"),
            precision: Some(4),
            symbol: "SYM",
            contract: Some("tract.token"),
        },
        ParseSet {
            text: "1.0000 SYM@tr.act.token",
            amount: Some("1.0000"),
            precision: Some(4),
            symbol: "SYM",
            contract: Some("tr.act.token"),
        },
        ParseSet {
            text: "1.0000 SYM",
            amount: Some("1.0000"),
    ...
    for test in parsetest {
        ...
        println!("test: {}\n=>{:?}", test.text, r);
        let flag = (r.amount.as_ref().map(String::as_str) == test.amount) &&
            (r.precision == test.precision) && (r.symbol == test.symbol) &&
            (r.contract.as_ref().map(String::as_str) == test.contract);
        assert!(flag, "erros");
    }
}
